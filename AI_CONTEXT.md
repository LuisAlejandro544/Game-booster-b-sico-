...
- **Centinela en Primer Plano (`GameWatcherService`)**: Monitorea con `dumpsys activity` / `cmd activity` si el juego sigue en pantalla. Cuando el usuario sale del juego, el servicio restaura instantáneamente la configuración normal de Android (resolución, densidad, `pm unsuspend`, `settings delete/put`, `am set-inactive false`, modo DND y banners heads-up).
...
- **Fail-Safe de Arranque (`BootRecoveryReceiver`)**: Restaura drivers, resolución/DPI de pantalla, estado de notificaciones DND/Heads-up y servicios de Google automáticamente si el teléfono se reinicia inesperadamente.
